...
class GT_Ambition_License_Key {
	...
	static function setup() {
		...
		// Define Product ID.
		define( 'GT_AMBITION_PRODUCT_ID', 336 );

		// Define Update API URL.
		define( 'GT_AMBITION_STORE_API_URL', 'https://germanthemes.de' );
	...
	static function activate_license() {
		...
		// Call the custom API.
		$response = wp_remote_post( GT_AMBITION_STORE_API_URL, array(
			'timeout'   => 25,
			'sslverify' => true,
			'body'      => $api_params,
		) );
		...
		update_option( 'gt_ambition_theme_options', $theme_options );

		delete_transient( 'gt_ambition_license_check' );
	...
	static function check_license() {
		...
		$status = get_transient( 'gt_ambition_license_check' );
		...
		if ( false === $status ) {
			...
			set_transient( 'gt_ambition_license_check', $status, DAY_IN_SECONDS );
		}
